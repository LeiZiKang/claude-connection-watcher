...
cleanup() {
  ...
  if [ -n "$notary_log_path" ] && [[ "$notary_log_path" == "$release_dir"/.*-notary-log.json ]]; then
...
archive_path="$release_dir/${base_name}.zip"
rm -f "$submission_path" "$notary_log_path" "$archive_path" "$archive_path.sha256"
...
/usr/bin/ditto -x -k "$archive_path" "$validation_root"
